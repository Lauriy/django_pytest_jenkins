                            ...
                            parallel(
                                testFirstApp: {
                                    database.withRun("--name django_pytest_jenkins_postgres_first -e 'POSTGRES_DB=django_pytest_jenkins_test_first' -e 'POSTGRES_USER=django_pytest_jenkins_test' -e 'POSTGRES_PASSWORD=django_pytest_jenkins_test' --network-alias postgres1 --net postgres-net") { c ->
                                        ...
                                        sh "docker run --name temp-container1 --entrypoint='' --user 0:0 -e DB_HOST=postgres1 --net postgres-net laurielias/django_pytest_jenkins:latest-test python -m pytest django_pytest_jenkins_tests/test_first_app.py --cov-report=xml:test-results/coverage1.xml --junitxml=test-results/pytest-report1.xml"
                                        sh "docker cp temp-container1:/srv/django_pytest_jenkins/test-results/ test-results/"
                                    }
                                },
                                testSecondApp: {
                                    database.withRun("--name django_pytest_jenkins_postgres_second -e 'POSTGRES_DB=django_pytest_jenkins_test_second' -e 'POSTGRES_USER=django_pytest_jenkins_test' -e 'POSTGRES_PASSWORD=django_pytest_jenkins_test' --network-alias postgres2 --net postgres-net") { c ->
                                        ...
                                        sh "docker run --name temp-container2 --entrypoint='' --user 0:0 -e DB_HOST=postgres2 --net postgres-net laurielias/django_pytest_jenkins:latest-test python -m pytest django_pytest_jenkins_tests/test_second_app.py --cov-report=xml:test-results/coverage2.xml --junitxml=test-results/pytest-report2.xml"
                                        sh "docker cp temp-container2:/srv/django_pytest_jenkins/test-results/ test-results/"
                                    }
                                }
                            )
                        }
